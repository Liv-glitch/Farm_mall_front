...
class ApiClient {
  ...
  constructor() {
    // Use base URL from config (already has environment variable with default)
  ...
  private async handleTokenRefresh() {
    try {
      ...
      const response = await this.client.post(config.auth.refreshEndpoint, {
  ...
  async register(data: RegisterRequest): Promise<any> {
    try {
      ...
      const response = await this.request<{
        ...
        method: "POST",
        url: config.auth.registerEndpoint,
        data,
      });
  ...
  async getProfile(): Promise<User> {
    return this.request({
      method: "GET",
      url: config.auth.profileEndpoint,
    })
  }
  ...
  async logout() {
    return this.request({
      method: "POST",
      url: config.auth.logoutEndpoint,
    })
  }
