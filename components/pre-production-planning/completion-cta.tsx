...
export function CompletionCta({ plan }: CompletionCtaProps) {
  // Hand the plan's details to the Production Cycles page so it can prefill a new cycle.
      ...
      <CardContent className="p-6 flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">
            Every pre-production step is complete. Start a production cycle, pre-filled with this plan&apos;s details.
          </p>
        </div>
        <Button asChild className="bg-agri-600 hover:bg-agri-700 w-full sm:w-auto">
          <Link href={href}>
            Start production cycle <ArrowRight className="ml-1.5 h-4 w-4" />
          </Link>
        </Button>
      </CardContent>
    </Card>
